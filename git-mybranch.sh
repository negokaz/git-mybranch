...
function mybranch/othrer_remote_refs {
    local current_branch current_remote excludes push_default ref_format='%(refname:short)'
    current_remote="$(git_silent branch --format='%(upstream:short)' --points-at HEAD)"
    if [ -z "${current_remote}" ]
    then
        push_default=$(git config --get --default simple push.default)
        if [ "${push_default}" = 'current' -o "${push_default}" = 'matching' ]
        then
            current_branch="$(git_silent symbolic-ref --short HEAD)"
            excludes=$(git remote | awk -v "branch=${current_branch}" '{ print $0 "/" branch }' | sort)
        fi
    else
        excludes="${current_remote}"
    fi
    comm -2 -3 <(git_silent for-each-ref --format="${ref_format}" refs/remotes | sort) <(echo "${excludes}")
}
